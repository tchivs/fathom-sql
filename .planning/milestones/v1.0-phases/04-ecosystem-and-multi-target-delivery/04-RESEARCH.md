...
## Resolved Decisions and Execution Probes

1. **Linear-Wasm byte transport — accepted decision:** exports return a documented serialized `Bytes` result through an explicit host-owned buffer/length ABI generated by the pinned toolchain. Wave 0 `binding/export_smoke_test.mbt` and `parity/run_wasm.mbt` probe the generated artifact; the implementation may select the exact pointer/length helper only within that ABI, and publication is blocked unless the host can decode the bytes and release ownership deterministically. No MoonBit object handles cross the boundary.

2. **JSON/byte schema — accepted decision:** `doris.parse.v1` and `doris.format.v1` use `source_transport: "inline-root-v1"`; the root carries the exact source bytes in the documented byte encoding, with source byte length and half-open byte spans. JS uses `Uint8Array`; JSON/LSP uses the schema's explicit byte representation. Any future omission or encoding change requires a new schema/transport version and rejection of unknown versions.

3. **Browser/runtime matrix — accepted decision:** JS ESM is the supported browser demo artifact and linear Wasm is tested in one checked-in Node host harness plus the browser's WebAssembly runtime used by the demo. Wasm GC, Deno, Wasmtime, and additional browser versions are not compatibility claims. The harness records target/toolchain and fails on decode/parity drift.

4. **VS Code packaging targets — accepted decision:** Phase 4 proves a local VSIX/client package with a documented user-configured Native executable path; it does not claim Marketplace publication or bundled multi-platform binaries. The human-hosted checkpoint must name the host VS Code version and executable path. No local `code` executable is assumed here.

5. **Granular formatting edits — accepted decision:** use one full-document `TextEdit` from byte 0 to source byte length. `statement_offsets` are output-only metadata and are never treated as source ranges. Granular edits are explicitly outside this phase's acceptance; the existing formatter output, comments/hints, refusal behavior, and centralized UTF-16 endpoints are tested instead.

These decisions close every prior open question. The Wave 0 harnesses are the bounded execution probes for toolchain-specific export syntax and host decoding; they do not delegate product choices or permit a second schema producer.
