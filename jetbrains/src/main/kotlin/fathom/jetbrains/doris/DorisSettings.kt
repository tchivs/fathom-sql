...
class DorisSettings : PersistentStateComponent<DorisSettings.State> {
    ...
    fun snapshot(): Configuration = Configuration(state.executablePath, state.profile)

    fun update(executablePath: String, profile: String) {
        ...
        state = State(normalizedPath, normalizedProfile)
    }

    data class Configuration(val executablePath: String, val profile: String)
