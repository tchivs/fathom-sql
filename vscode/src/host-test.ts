...
//   functional profile=4.x  -> diagnostics / format / completion / MERGE ok
//   profile    profile=2.1  -> MERGE rejected (DORIS-PARSE-006) = profile propagation
...
const LSP_PATH = process.env.DORIS_LSP_PATH || '/opt/source/Fathom/_build/native/debug/build/lsp/lsp.exe';
const BAD_PATH = process.env.DORIS_LSP_PATH_BAD || '/nonexistent/doris-lsp-missing';
const WORKSPACE = process.env.DORIS_WORKSPACE || '/tmp/doris-host-workspace';
...
async function setConfig(serverPath, profile) {
  const config = vscode.workspace.getConfiguration('doris');
...
async function runFunctional() {
  ...
  assert.ok(first.source === 'doris' || first.source === 'doris-sql', `source is doris, got ${first.source}`);
  ...
  assert.ok(!mergeDiags.some((d) => d.code === 'DORIS-PARSE-006'), '4.x MERGE has no version-invalid diagnostic');

  console.log('HOST-FUNCTIONAL: diagnostics/format/completion/4.x-merge passed');
}

async function runProfile() {
  await setConfig(LSP_PATH, '2.1');

  // MERGE is introduced at 4.x (D-09) -> 2.1 must report DORIS-PARSE-006,
  ...
  const diags = await waitFor(async () => {
    ...
    return d.some((x) => x.code === 'DORIS-PARSE-006') ? d : null;
  });
  assert.ok(diags.some((d) => d.code === 'DORIS-PARSE-006'), '2.1 MERGE reports DORIS-PARSE-006 (profile propagation)');

  console.log('HOST-PROFILE: 2.1 rejects MERGE via DORIS-PARSE-006 (profile propagated)');
}

async function runFallback() {
  await setConfig(BAD_PATH, '4.x');

  // Opening a Doris document must not crash; server failure surfaces and the
...
export async function run() {
  assert.ok(LSP_PATH, 'DORIS_LSP_PATH must be set');
