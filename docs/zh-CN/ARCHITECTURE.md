...
`moon.mod` 将模块声明为 `fathom/doris-sql`，版本为 `0.1.0`，首选目标为 `native`；核心库包的 manifest 显式声明为 `pkgtype(kind: "library")`，`test/moon.pkg` 未声明包类型；`doris-sql/moon.pkg` 声明 `pkgtype(kind: "executable")`，提供薄 CLI 适配器。`moon.mod` 中记录的 MoonBit 工具链版本为 `moon 0.1.20260724`。
